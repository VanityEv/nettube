...
const getVideosByGenre = async (genre, requestCallback) => {
  try {
    // Case-insensitive search that handles comma-separated genres
    ...
    // Additional filtering on the application side for exact genre matches
    // This handles cases where "Music" shouldn't match "Musical" in comma-separated values
    const filteredVideos = videos.filter(video => {
      if (!video.genre) return false;
      
      const videoGenres = video.genre.split(',').map(g => g.trim().toLowerCase());
      return videoGenres.some(g => g === genre.toLowerCase());
    });
    
    requestCallback(filteredVideos);
  } catch (error) {
    requestCallback({ error: error.message });
  }
};
