...
import { setUserData, setAvatarUrl } from '../../../store/slices/userSlice';
...
export const PersonalData = () => {
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        const response = await axios.post(`${api}/user/uploadAvatar/${username}`, formData, {
          headers: { Authorization: `Bearer ${getCookie('userToken')}` },
        });

        // If upload was successful, update the avatar URL in Redux store
        if (response.data.result === 'SUCCESS' && response.data.avatarUrl) {
          dispatch(setAvatarUrl(response.data.avatarUrl));
        }

        // Refetch user data to get updated avatar URL
        refetch();
        // Also update the userStore to refresh the AppBar avatar
        await dispatch(setUserData(username));
      } catch (error) {
